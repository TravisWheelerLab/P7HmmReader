...
void testHmmHeader(struct P7Hmm *phmm, const char *expectedVersion, const char *expectedName,
  ...
  const float expectedForwardStats[2]){
    ...
    sprintf(printBuffer, "expected msv gumble mu %f, but got %f.", expectedMsvStats[0], phmm->stats.msvGumbelMu);
    testAssertString(floatCompare(expectedMsvStats[0], phmm->stats.msvGumbelMu), printBuffer);

    sprintf(printBuffer, "expected msv gumble lambda %f, but got %f.", expectedMsvStats[1], phmm->stats.msvGumbleLambda);
    testAssertString(floatCompare(expectedMsvStats[1], phmm->stats.msvGumbleLambda), printBuffer);

    sprintf(printBuffer, "expected viterbi gumble mu %f, but got %f.", expectedViterbiStats[0], phmm->stats.viterbiGumbleMu);
    testAssertString(floatCompare(expectedViterbiStats[0], phmm->stats.viterbiGumbleMu), printBuffer);

    sprintf(printBuffer, "expected viterbi gumble lambda %f, but got %f.", expectedViterbiStats[1], phmm->stats.viterbiGumbleLambda);
    testAssertString(floatCompare(expectedViterbiStats[1], phmm->stats.viterbiGumbleLambda), printBuffer);

    sprintf(printBuffer, "expected forward gumble tau %f, but got %f.", expectedForwardStats[0], phmm->stats.forwardTau);
    testAssertString(floatCompare(expectedForwardStats[0], phmm->stats.forwardTau), printBuffer);

    sprintf(printBuffer, "expected forward gumble lambda %f, but got %f.", expectedForwardStats[1], phmm->stats.forwardLambda);
